...
get_direct_control = function(N = 100, tol = 1e-10, fill_method = "small_rects",
	max_rejects = 0, priority_weight = 0.5)
{
	ret = list(N_direct = N, fill_method = fill_method, tol = tol,
		max_rejects = max_rejects, priority_weight = priority_weight)
...
gibbs = function(y, X, init, prior, control, fixed = get_fixed())
{
	n = length(y)
	d = ncol(X)
	stopifnot(n == nrow(X))
	...
	nu_rejects_hist = numeric(R_keep)
	...
	for (rep in 1:R) {
		...
		if (!fixed$nu) {
			...
			ds_out = r_tdist_unif(n = 1, m = n, A = A,
				...
				max_rejects = direct_options$max_rejects)
			nu = ds_out$x
			nu_rejects = ds_out$rejects
		}
		...
		if (rep > burn && rep %% thin == 0) {
			...
			nu_hist[rep_keep] = nu
			nu_rejects_hist[rep_keep] = nu_rejects
			if (control$save_s) {
				s_hist[rep_keep,] = s
			}
		}
	...
	ret = list(beta_hist = beta_hist, sigma2_hist = sigma2_hist, nu_hist = nu_hist,
		...
		thin = thin, nu_rejects_hist = nu_rejects_hist)
	class(ret) = "gibbs_fit"
	return(ret)
}
...
print.gibbs_fit = function(x, ...)
{
	...
	printf("Rejections in nu step: %d  Total proposals: %d  Rejection rate: %0.4f%%\n",
		sum(x$nu_rejects_hist),
		sum(x$nu_rejects_hist) + x$R,
		100 * sum(x$nu_rejects_hist) / (sum(x$nu_rejects_hist) + x$R))
