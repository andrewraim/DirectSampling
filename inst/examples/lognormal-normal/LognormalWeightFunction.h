...
class LognormalWeightFunction : public DirectSampling::WeightFunction
{
public:
	LognormalWeightFunction(double z, double mu, double sigma2)
		: DirectSampling::WeightFunction(), _z(z), _mu(mu), _sigma2(sigma2)
	{
	}
	...
	double eval(double x, bool log = false) const {
		double out = R_NegInf;
		if (x < _z) {
			out = -std::log(_z-x) - pow(std::log(_z-x) - _mu, 2) / (2*_sigma2);
		}
