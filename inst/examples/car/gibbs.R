...
get_direct_control = function(N = 100, max_rejects = 0)
{
	ret = list(N = N, max_rejects = max_rejects)
...
gibbs = function(y, X, S, A, init, prior, control, fixed = get_fixed())
{
	n = length(y)
	d = ncol(X)
	k = ncol(S)
	...
	for (rep in 1:R) {
		...
		if (!fixed$rho) {
			...
			ds_out = r_car_beta(n = 1, lambda = lambda, C = C,
				...
				N = direct_control$N,
				max_rejects = direct_control$max_rejects)
			rho = ds_out$x
			rho_rejections = ds_out$rejects
